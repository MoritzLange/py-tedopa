...
def evolve(state, hamiltonians, t, num_trotter_slices, method, compr):
    ...
    Args:
        ...
        t (list of floats): The times for which the evolution should be computed
    ...
    if (np.array(t) == 0).all(): raise ValueError("No time evolution requested by the user. Check your input 't'")

    tau = _times_to_steps(t, num_trotter_slices)

    u = _trotter(hamiltonians=hamiltonians, tau=tau, num_sites=len(state))
    _compress(u, method)

    return _time_evolution(state, u, t, tau, method, compr)


def _time_evolution(state, u, t, tau, method, compr):
    """
    ...
    Args:
        ...
        t (list): List of time steps as generated by _times_to_steps()
    ...
    times = [0] if t[0] == 0 else times = []
    states = [state] if t[0] == 0 else states = []
    compr_errors = [0] if t[0] == 0 else compr_errors = []
    trot_errors = [0] if t[0] == 0 else trot_errors = []
...
def _times_to_steps(times, num_trotter_slices):
    """
    ### CAREFUL: THIS FUNCTION MODIFIES times IN PLACE DESPITE RETURNING A VALUE! ###
    Convert the given times in evolve(), for which a state is requested, into steps (in place) and return tau,
    which is the maximal time from times divided by num_trotter_slices
    Args:
        times (list of floats): The times for which the evolution should be computed
        num_trotter_slices (int): The number of time steps or Trotter slices for the time evolution

    Returns:
        tau (float): maximal t / num_trotter_slices
...
def _trotter(hamiltonians, tau, num_sites):
    """
...
def _get_u_list(h_single, h_adjacent, tau):
    """
    Calculate time evolution operators from Hamiltonians. The time evolution operators acting on odd sites
    ...
    Returns:
        list: List of dimensions of each site and lists of time evolution operators.
            The second list are operators acting on odd adjacent sites and the third one
            the ones acting on even adjacent sites, like [u12, u34, ...], [u23, u45, ...]

    """
    dims = [len(h_single[i]) for i in range(len(h_single))]
    h_2sites = [np.kron(h_single[i], np.identity(len(h_single[i + 1]))) + np.kron(np.identity(len(h_single[i])),
                                                                                  h_single[i + 1])
                for i in range(0, len(h_single) - 1, 2)]
    u_odd = list(expm(-1j * tau / 2 * (h + h_2sites[i])) for i, h in enumerate(h_adjacent[::2]))
    u_even = list(expm(-1j * tau * h) for h in h_adjacent[1::2])
    if len(u_odd) == len(u_even): u_odd = u_odd + [expm(-1j * tau / 2 * h_single[-1])]
...
def _u_list_to_mpo(dims, u_odd, u_even):
    ...
    Returns:
        mpnum.MPArray: The time evolution MPOs for the full state acting on odd adjacent sites
            and acting on even adjacent sites.
...
def matrix_to_mpo(matrix, shape):
    """
    Generates a MPO from a NxN matrix in global form (probably also works for MxN).
    ...
    Args:
        ...
        shape (list): The shape the single sites of the resulting MPO should have, as used in mpnum.
            For example three sites with two legs each might look like [[3, 3], [2, 2], [2, 2]]
...
def _compress(state, method):
    """
    Compress and normalize a state with a very small relative error. This is meant to get unnecessary ranks out of
